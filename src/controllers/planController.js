...
import {
  generateFlashcards,
  normalizeFlashcards,
} from "../services/flashcardGenerator.js";
  ...
  try {
    ...
    if (!kit.questions?.length)
      return res
        ...
        .json({
          success: false,
          message: "Generate interview questions before creating a study plan.",
        });
    ...
    try {
      kit.flashcards = normalizeFlashcards(
      ...
      console.error("Flashcard generation failed; saving schedule only", {
    ...
    return res.json({
      success: true,
      data: { kit, partial: !kit.flashcards.length },
    });
  } catch (error) {
